...
function changeRect(grid: number[][], start: [number, number], end: [number, number], value: number): void {
    ...
    // instead of using value, use updateValue
    ...
    //         // [...fill]
    //         ...rowFill,
    //         updateValue()...grid[offset].slice(0, start[0], )),
    //         // [...old]]
    ...
    for (let col = start[0]; col <= end[0]; col++) {
        for (let row = start[1]; row <= end[1]; row++) {
            grid[row][col] = Math.max(0, grid[row][col] + value);
        }
    }
}
...
function runner(instructions: Instruction[]): number {
    ...
    instructions.forEach((ins) => {
        switch (ins.action) {
            case 'toggle':
                ...
                changeRect(grid, ins.start, ins.end, 2);
                break;
            case 'turn off':
    ...
    return grid.flat().reduce((sum, v) => sum + v);
}
